...
setwd("path") # set thr working directory to that containing your fastq files
...
length(File_Forward) == length(File_Reverse) # check whether the number of forward is equal to that of the reverse

Sample_Name <- sapply(strsplit(File_Forward, "_"), "[[", 1) # get the sample names
length(Sample_Name) # check the number of samples

length(File_Forward) == length(Sample_Name) # confirm that the number of forward reads is equal to the number of samples
